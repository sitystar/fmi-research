// Патч: Evgenii I, 2026 — адаптация к Boost 1.90 / CMake 4 / GCC 15 и поддержка целочисленных ASN.1-тегов IEC 61499. Полный перечень правок: PATCHES.md
/* ------------------------------------------------------------------- *
	...
	std::shared_ptr<Timing::EventSink> eventSink)
{
	...
	busyKeeper_ = std::unique_ptr<boost::asio::executor_work_guard<
			boost::asio::io_context::executor_type>>(
			new boost::asio::executor_work_guard<
				boost::asio::io_context::executor_type>(
				boost::asio::make_work_guard(service_)));
...
{
	boost::asio::post(service_, boost::bind(&CompactASN1Subscriber::handleTermination, this));
}

boost::asio::io_context* CompactASN1Subscriber::getIOService()
{
	return &service_;
}
...
CompactASN1Subscriber::readNextVariable(boost::any *dest)
{
	...
	case ASN1Commons::CLASS_APPLICATION | ASN1Commons::DINT_TAG_NR:
		...
		break;
	case ASN1Commons::CLASS_APPLICATION | ASN1Commons::SINT_TAG_NR:
		state = readAndConvertVariable<DestinationType, int8_t>(
			&CompactASN1Subscriber::readXINTVariable<int8_t>, dest);
		break;
	case ASN1Commons::CLASS_APPLICATION | ASN1Commons::INT_TAG_NR:
		state = readAndConvertVariable<DestinationType, int16_t>(
			&CompactASN1Subscriber::readXINTVariable<int16_t>, dest);
		break;
	case ASN1Commons::CLASS_APPLICATION | ASN1Commons::LINT_TAG_NR:
		state = readAndConvertVariable<DestinationType, int64_t>(
			&CompactASN1Subscriber::readXINTVariable<int64_t>, dest);
		break;
	case ASN1Commons::CLASS_APPLICATION | ASN1Commons::USINT_TAG_NR:
		state = readAndConvertVariable<DestinationType, uint8_t>(
			&CompactASN1Subscriber::readXINTVariable<uint8_t>, dest);
		break;
	case ASN1Commons::CLASS_APPLICATION | ASN1Commons::UINT_TAG_NR:
		state = readAndConvertVariable<DestinationType, uint16_t>(
			&CompactASN1Subscriber::readXINTVariable<uint16_t>, dest);
		break;
	case ASN1Commons::CLASS_APPLICATION | ASN1Commons::UDINT_TAG_NR:
		state = readAndConvertVariable<DestinationType, uint32_t>(
			&CompactASN1Subscriber::readXINTVariable<uint32_t>, dest);
		break;
	case ASN1Commons::CLASS_APPLICATION | ASN1Commons::ULINT_TAG_NR:
		state = readAndConvertVariable<DestinationType, uint64_t>(
			&CompactASN1Subscriber::readXINTVariable<uint64_t>, dest);
		break;
	default:
...
CompactASN1Subscriber::readXREALVariable(FloatType *dest)
{
	...
	else
	{
		...
		const UIntType *buffer = static_cast<const UIntType*>(remainingRawData_.data().data());
...
CompactASN1Subscriber::readSTRINGVariable(std::string *dest)
{
	...
	buffer = static_cast<const uint8_t *>(remainingRawData_.data().data());
	buffer++;
	...
	const char *str = static_cast<const char *>(remainingRawData_.data().data());
...
CompactASN1Subscriber::readXINTVariable(IntType *dest)
{
	...
	const IntType *dat = static_cast<const IntType*>(remainingRawData_.data().data());
