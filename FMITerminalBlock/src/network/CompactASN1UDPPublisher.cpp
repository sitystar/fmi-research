...
void CompactASN1UDPPublisher::init(const Base::TransmissionChannel &channel)
{
	...
	// Resolve throws boost::system::system_error (subclass of
	// std::runtime_error) on failure; results range has at least one element
	destination_ = *resolver.resolve(
		addr.substr(0, sepPos), addr.substr(sepPos + 1)).begin();
